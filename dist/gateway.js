...
export function createGateway(options) {
    const { appId, clientSecret, onMessage, logger: log, reconnectDelays = DEFAULT_RECONNECT_DELAYS, maxReconnectAttempts = MAX_RECONNECT_ATTEMPTS, } = options;
    let ws = null;
        ...
        try {
            ...
            ws.on("message", (data) => {
                try {
                    ...
                    switch (payload.op) {
                        case 10: // Hello
                            const heartbeatIntervalMs = payload.d?.heartbeat_interval ?? 41250;
