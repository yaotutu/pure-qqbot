 ...
 * 提供简洁的消息收发接口
 */
import { getAccessToken, sendC2CMessage, sendGroupMessage, sendProactiveC2CMessage, sendProactiveGroupMessage, clearTokenCache, 
// 富媒体
sendC2CImageMessage, sendGroupImageMessage, sendC2CVoiceMessage, sendGroupVoiceMessage, sendC2CVideoMessage, sendGroupVideoMessage, sendC2CFileMessage, sendGroupFileMessage, } from "./api.js";
...
export class QQBotClient {
     ...
     * @param content 消息内容
     */
    async sendPrivateMessageProactive(openid, content) {
     ...
     * @param content 消息内容
     */
    async sendGroupMessageProactive(groupOpenid, content) {
     ...
     */
    async reply(event, content, useProactive = false) {
     ...
     */
    async sendPrivateImage(openid, imageUrl, msgId, content) {
     ...
     * @param content 附带的文本内容（可选）
     */
    async sendGroupImage(groupOpenid, imageUrl, msgId, content) {
     ...
     * @param msgId 要回复的消息ID（被动回复时必填）
     */
    async sendPrivateVoice(openid, voiceBase64, voiceUrl, msgId) {
     ...
     * @param msgId 要回复的消息ID（被动回复时必填）
     */
    async sendGroupVoice(groupOpenid, voiceBase64, voiceUrl, msgId) {
     ...
     * @param content 附带的文本内容（可选）
     */
    async sendPrivateVideo(openid, videoUrl, videoBase64, msgId, content) {
     ...
     * @param content 附带的文本内容（可选）
     */
    async sendGroupVideo(groupOpenid, videoUrl, videoBase64, msgId, content) {
     ...
     * @param msgId 要回复的消息ID（被动回复时必填）
     */
    async sendPrivateFile(openid, fileBase64, fileUrl, fileName, msgId) {
     ...
     * @param msgId 要回复的消息ID（被动回复时必填）
     */
    async sendGroupFile(groupOpenid, fileBase64, fileUrl, fileName, msgId) {
